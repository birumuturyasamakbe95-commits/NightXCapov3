...
function M.accentColor()
    return M.getUiColorEntry(M.uiColorName or "Black").accent
end
...
function M.buildMobileButtons()
    ...
    local function setMobileButton(key, on)
        ...
        local oceanLite = Color3.fromRGB(220, 220, 220)
...
M.stealBarSize = tonumber(M.stealBarSize) or 420
    ...
    if not ok then
        ...
        warn("[Capo X Nighthub V3] CherryConfig.json was corrupt — reset (backup: CherryConfig.bad.json)")
        d = nil
    end
...
function saveCherryConfig()
    ...
    local cfg = {
        ...
        unwalkEnabled=M.unwalkEnabled==true, antiDieEnabled=M.antiDieEnabled==true, antiFlingEnabled=M.antiFlingEnabled==true, antiFling=M.antiFlingEnabled==true, antiLag=M.antiLagEnabled==true, antiLagEnabled=M.antiLagEnabled==true,
        antiBatEnabled=M.antiBatEnabled==true,
        holdJumpEnabled=M.holdJumpEnabled==true,
        antiDropEnabled=M.antiDropEnabled==true, stretchEnabled=M.stretchEnabled==true, stretch=M.stretchEnabled==true, antiSummerBase=M.antiSummerBaseEnabled==true, antiSummerBaseEnabled=M.antiSummerBaseEnabled==true, uiLocked=M.uiLocked,
...
function M.applyChromeTheme()
    ...
    local strokeCol = Color3.fromRGB(200, 200, 210)
    ...
    if hdr and hdr.Parent then
        ...
        if vh and vh.Parent then
            pcall(function()
                ...
                if vLbl then
                    vLbl.Text = "C"
                    vLbl.TextColor3 = Color3.fromRGB(200, 200, 210)
                    vLbl.TextSize = 28
                end
